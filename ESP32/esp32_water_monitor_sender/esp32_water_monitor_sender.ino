...
constexpr uint32_t POST_INTERVAL_MS = 2000;         // 2000ms - stable real-time updates (reduced from 200ms)
...
void updateLcd(const Reading &r, uint8_t cycleIndex) {
  ...
  // Handle offline state - show only "ESP32 Offline" with blank line 2
  if (WiFi.status() != WL_CONNECTED) {
    lcd.setCursor(0, 0);
    lcd.print("ESP32 Offline");
    // Line 2 left blank
    return;
  }
  ...
  if (!r.hasWater) {
    ...
    lcd.setCursor(0, 1);
    lcd.printf("Lv:%.0f%% Vol:--L", r.tankLevelPercent);
    return;
  }

  // ALWAYS show Level and Volume on line 2
  lcd.setCursor(0, 1);
  lcd.printf("Lv:%2.0f%% Vol:%.1fL", r.tankLevelPercent, 
             r.tankCapacity * (r.tankLevelPercent / 100.0f));

  // Priority 1: Flow detection (show first when active)
  ...
  // Normal cycling (2 cycles only)
  switch (cycleIndex % 2) {
    case 0:  // Temperature and pH
      lcd.setCursor(0, 0);
      lcd.printf("Temp:%2.1fC pH:%2.1f", r.temperatureC, r.ph);
      break;
      
    case 1:  // Turbidity and Color Sensor (show R value due to space)
      lcd.setCursor(0, 0);
...
void loop() {
  ...
  if (now - lastPostMs >= POST_INTERVAL_MS || lastPostMs == 0) {
    ...
    // Send telemetry to dashboard if WiFi is connected
    if (WiFi.status() == WL_CONNECTED) {
      String payload;
      buildTelemetryPayload(latestReading, timestamp, recordId, payload);
      sendPayloadToDashboard(payload);
    }
    // If offline, data is still recorded to SD card in CSV format
